...
export class AuthService {
  ...
  createUser(fname: string, lname: string, email: string, pass: string, cpass: string) {
    ...
    .set('password', pass);
    this.http
      .post('http://ec2-54-189-138-26.us-west-2.compute.amazonaws.com:3000/api/user/register', payload)
      .subscribe(() => {
        this.loggedIn = true;
  ...
  login(emal: string, pass: string) {
    ...
    this.http
      .post<{ token: string; expiresIn: number; }>(
        'http://ec2-54-189-138-26.us-west-2.compute.amazonaws.com:3000/api/user/login', payload)
      .subscribe(response => {
